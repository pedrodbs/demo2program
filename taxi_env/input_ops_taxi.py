...
def check_data_id(dataset, data_id):
    ...
    if len(wrong) > 0:
        raise RuntimeError(f'There are {len(wrong):d} invalid ids, including {wrong[:5]}')
...
def create_input_ops(dataset,
    ...
    if data_id is None:
        data_id = dataset.ids
        log.info(f'input_ops [{scope}]: Using {len(data_id)} IDs from dataset')
    else:
        log.info(f'input_ops [{scope}]: Using specified {len(data_id)} IDs')
    ...
    with tf.device('/cpu:0'), tf.name_scope(scope):
        ...
        p, pt, s, ts, a, at, ta, tat, pl, dl, tdl, per, tper = dataset.get_data(data_id[0])
